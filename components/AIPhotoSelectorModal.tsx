...
interface AIPhotoSelectorModalProps {
  onClose: () => void;
...
export const AIPhotoSelectorModal: React.FC<AIPhotoSelectorModalProps> = ({ onClose, onSetProfilePicture }) => {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            ...
            if (files.length > MAX_PHOTOS) {
                ...
                setSelectedFiles(files.slice(0, MAX_PHOTOS));
            } else {
        ...
        <div className="space-y-6">
            ...
            {imagePreviews.length > 0 && (
                <div className="space-y-4">
                    <h4 className="font-semibold text-deep-teal">Selected Photos:</h4>
                        ...
                        return (
                            <div key={index} className={`bg-white p-4 rounded-xl shadow-md border-2 ${isBest ? 'border-rose-gold' : 'border-transparent'}`}>
                                ...
                                <div className="mt-4">
                                    ...
                                    {isBest && (
                                        <Button className="w-full mt-4" onClick={() => onSetProfilePicture(src)}>Set as Profile Picture</Button>
                                    )}
                                </div>
                            </div>
                        );
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
                <div className="flex justify-between items-center p-6 border-b border-slate-200">
                    <h2 className="text-2xl font-bold font-serif text-deep-teal">AI Profile Photo Selector</h2>
